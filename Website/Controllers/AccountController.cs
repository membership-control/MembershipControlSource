...
namespace WebMembership.Controllers
{
    ...
    public class AccountController : BaseController //Controller
    {
        ...
        [HttpPost]
        [AllowAnonymous]
        public async Task<ActionResult> RemoteLogin(LoginViewModel model,string clientKey)
        {
            if (!ModelState.IsValid)
            {
                return View(model);
            }
            SignInManager.ClientKey = "Remote:" + clientKey;
            var result = await SignInManager.PasswordSignInAsync(model.Email, model.Password, model.RememberMe, shouldLockout: false);
        ...
        [AllowAnonymous]
        public JsonResult IsAvailableUser(string Email)
        {
            var isexist = UserManager.Users.Where(predicate => predicate.Email == Email).Count();
            if (isexist == 0)
        ...
        public async Task<ActionResult> ConfirmEmail(string userId, string code)
        {
            ...
            if (result.Succeeded)
            {
                UserManager.AddToRole(userId, "Default User");
            }
